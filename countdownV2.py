...
def ensure(event):
    ...
    global s
    if (
        (str(entry_hour.get()).isdigit() or str(entry_hour.get()) == "")
        and (str(entry_minute.get()).isdigit() or str(entry_minute.get()) == "")
        and (str(entry_second.get()).isdigit() or str(entry_second.get()) == "")
    ):
...
music = AudioSegment.from_mp3("2.mp3")
setcount()
countdown()
